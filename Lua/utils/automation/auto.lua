...
function Auto.LoadLuaFile(filePath, modName)
  ...
  if filePath:lower():find("megamod_autorun") then
    str = "\n"
  end
...
elseif Auto.Step == 2 then -- Notify of mod updates, apply Lua/xml overrides, rewrite xml to correct file paths, rewrite filelist, rewrite .bat to copy Lua directories
  ...
  for modDirectory in File.GetDirectories(Auto.Path .. "/Workshop") do
    ...
    for folder in File.GetDirectories(modDirectory) do
      if Auto.GetFileName(folder, true) == "Lua" then
        local autorunDir
        local function searchForAutorunDir(directory)
          if autorunDir then return end
          local dirName = string.lower(Auto.GetFileName(directory, true))
          if dirName == "autorun" or dirName == "forcedautorun" then
            autorunDir = directory
            return
          end
          for childDirectory in File.GetDirectories(directory) do
            searchForAutorunDir(childDirectory)
          end
        end
        searchForAutorunDir(folder)
        if autorunDir then
          ...
          print("WARNING! No autorun directory found for " .. tostring(modName))
        end
        break
      end
    end
  ...
  File.Write(Auto.Path .. "/Lua/utils/automation/autorunpaths.json", json.encode(autorunPaths))
