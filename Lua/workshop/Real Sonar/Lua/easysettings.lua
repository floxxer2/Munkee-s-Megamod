...
Hook.Patch("Barotrauma.GUI", "TogglePauseMenu", {}, function ()
    ...
    -- The else block runs on menu close.
        ...
        if Game.IsMultiplayer then
            local message = Networking.Start("realsonar_reloadserverconfig")
            Networking.Send(message)
        end

        if easySettings.needSonarItemsUpdate then
            if Game.IsMultiplayer then
                local message = Networking.Start("realsonar_getsonaritems")
                Networking.Send(message)
            else
        ...
        if easySettings.needWearableProtectionsUpdate then
            if Game.IsMultiplayer then
                local message = Networking.Start("realsonar_setitemtags")
                Networking.Send(message)
            else
