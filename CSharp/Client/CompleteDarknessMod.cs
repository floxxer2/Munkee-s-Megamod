    ...
    class CompleteDarknessMod : IAssemblyPlugin
    {
        ...
        public static bool RenderLightMap(GraphicsDevice graphics, SpriteBatch spriteBatch, Camera cam, Barotrauma.Lights.LightManager __instance, RenderTarget2D backgroundObstructor = null)
        {
            ...
            foreach (LightSource light in _.lights)
            {
                ...
                //Munkee code - don't render lights that are not hunt immune if a Hunt is active
                if (HuntActive &&
                (light.ParentBody == null //Static lights - never hunt immune
                || light.ParentBody?.UserData is not Item //Character lights?
                || light.ParentBody?.UserData is Item it && !it.HasTag("huntimmune")))
                {
                    continue;
                }
